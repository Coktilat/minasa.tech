 ...
 */
$title = $values = $units = $bgcolor = $css = $custombgcolor = $customtxtcolor = $options = $el_class = $margin_bottom_barra = $bar_txt_size = '';
$output = '';
...
$css_class.= $bar_txt_size;

$output = '<div id="pbar-'.rand(1, 500).'" class="'.esc_attr( $css_class ).'">';
...
$max_value = 0.0;
$inline=' style="';
if (isset($custombgcolor_back) && $custombgcolor_back!="") {
    $inline.='background-color: '.$custombgcolor_back.';';
}
if ($margin_bottom_barra != "") {
	$inline.='margin-bottom: '.$margin_bottom_barra.';'; 
}
if ($inline!=' style="') {
	$inline.='"';
}
...
foreach ( $graph_lines_data as $line ) {
	$unit = ( '' !== $units ) ? ' <span class="vc_label_units wpb_animate_when_almost_visible wpb_hook_fade_waypoint delay-100"'.$line['txtcolor'].'>'.$line['value'].$units.'</span>' : '';
	if ( $max_value > 100.00 ) {
	...
	$output .= '<div class="vc_general vc_single_bar'.( ( isset( $line['color'] ) && 'custom' !== $line['color'] ) ?
		...
		. '"'.$inline.'>';
	$output .= '<div class="vc_label"'.$line['txtcolor'].'><div class="hook_liner">'.$line['label'].'</div></div>';
