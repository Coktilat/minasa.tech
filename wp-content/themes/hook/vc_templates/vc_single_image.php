 ...
 */
$title = $source = $image = $custom_src = $onclick = $img_size = $css_delay = $external_img_size =
$caption = $img_link_large = $link = $img_link_target = $alignment = $el_class = $css_animation = $style = $external_style = $border_color = $css = $custom_css = $retina_image = $caption_style = $font_type = $custom_color = $add_border = '';
...
$class_to_filter = 'wpb_single_image vc_align_'.$alignment.' '.$this->getCSSAnimation( $css_animation );

//DELAY SUPPORT
if (isset($atts['css_delay'])) {
    $class_to_filter.=' delay-'.$atts['css_delay'];
}
...
if ( 'yes' === $add_caption && '' !== $caption ) {
    $caption_span="";
    if ($caption_style==" hook_caption_in") {
        $caption_css=$font_type." site_background_colored prk_heavier_600";
    }
    else if ($caption_style==" hook_caption_box") {
        $caption_css=$font_type." zero_color";
        $caption_span=' class="hook_bk_site"';
    }
    else {
        $caption_css="small_headings_color hook_italic prk_9_em";
    }
    $inline_code=$inline_span_code="";
    if ($custom_color!="") {
        $inline_code.=' style="color:'.$custom_color.';"';
        if ($add_border=='no') {
            $inline_span_code .= ' style="border-width:0px;"';
        }
        else {
            $inline_span_code .= ' style="border-color:'.$custom_color.';"';
        }
    }
    $html = '
		<figure class="vc_figure">
			'.$html.'
			<figcaption class="vc_figure-caption hook_center_align '.$caption_css.'"'.$inline_code.'><span'.$caption_span.$inline_span_code.'>'.esc_html( $caption ).'</span></figcaption>
		</figure>
	';
}
$css_class.=$caption_style;

//BETTER SVG IMAGE SUPPORT
$array = explode('.', $img['p_img_large'][0]);
$extension = end($array);
if($extension=='svg') {
	$css_class.=' hook_svg';
}
...
if ($retina_image=="yes") {
	$css_class.=' hook_retina';
}

//CUSTOM CSS SUPPORT
$hook_custom_css="";
if ($custom_css!="") {
	$hook_custom_css=' style="'.$custom_css.'"';
}

$output = '
	<div class="'.esc_attr( trim( $css_class ) ).'"'.$hook_custom_css.'>
