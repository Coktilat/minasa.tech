...
*/
get_header();
if (get_field('blog_layout')=="masonry") {
    //MASONRY LAYOUT
    ...
    ?>
    <div id="hook_ajax_inner" class="hook_colored page-prk-blog-masonry<?php echo esc_attr($hook_featured_style); ?>" data-color="<?php echo esc_attr($hook_featured_color); ?>">
        <div id="hook_content">
            <?php
            ...
            <div id="hook_content_inner">
                <div class="small-12 small-centered columns row prk_inner_block">
                    ...
                    if ($hook_query->have_posts()) :
                        ...
                        if (get_field('show_filter')=="1") {
                            ...
                            if ($hook_count > 0) {
                                foreach ( $hook_terms as $hook_term ) {
                                    if (in_array($hook_term->slug, $hook_filter_array) !== false || $hook_inside_filter=="")
                                        $hook_out.='<li class="small b_filter"><a class="'.$hook_term->slug.'" data-filter="'.$hook_term->slug.'" href="javascript:void(0)" data-color="'.esc_attr($prk_hook_options['bd_headings_color']).'">'.$hook_term->name.'</a></li>';
                                }
                            }
                            $hook_out.='</ul>';
                            $hook_out.='</div>';
                        ...
                        $hook_out.='<div class="masonry_blog per_init prk_section clearfix">';
                        ...
                        while ($hook_query->have_posts()) : $hook_query->the_post();
                            ...
                            if ($hook_post_counter<=$posts_per_page) {
                                ...
                                $hook_out.='<div class="masonry_inner prk_bordered">';
                                if (has_post_thumbnail($post->ID)) {
                                    ...
                                    $hook_out.='</div>';
                                    $hook_vt_image=vt_resize( get_post_thumbnail_id($post->ID),'', 680, 0, false , $hook_retina_device );
                                ...
                                $hook_out.=hook_excerpt_dynamic(22,$post->ID);
                                $hook_out.='<div class="clearfix"></div>';
                                $hook_out.='</div>';
                                $hook_out.='</div>';
                                ...
                                if ($prk_hook_options['postedby_blog']=="1") {
                                    if (function_exists('get_avatar')) {
                                        ...
                                        $hook_out.='<a href="'.get_author_posts_url($post->post_author).'" class="hook_anchor">';
                                        if (get_the_author_meta('prk_author_custom_avatar',$post->post_author)!="") {
                                            $hook_vt_image=vt_resize('', get_the_author_meta('prk_author_custom_avatar',$post->post_author), 56, 0, false, false);
                                            $hook_out.='<img src="'.esc_url($hook_vt_image['url']).'" width="'.esc_attr($hook_vt_image['width']).'" height="'.esc_attr($hook_vt_image['height']).'" alt="'.hook_alt_tag(false,get_the_author_meta('prk_author_custom_avatar',$post->post_author)).'" />';
                                        }
                                        else {
                                            if (get_avatar(get_the_author_meta('email',$post->post_author),'216')) {
                                                $hook_out.=get_avatar(get_the_author_meta('email',$post->post_author),'216');
                                            }
                                        }
                                        $hook_out.='</a>';
                                        $hook_out.='</div>';
                                    }
                                    $hook_out.='<div class="hook_anchor prk_lf hook_colored_link hook_author">';
                                    $hook_out.=esc_attr($hook_translated['posted_by_text']).' <a href="'.get_author_posts_url($post->post_author).'" class="not_zero_color" data-id="'.$post->post_author.'">'.get_the_author_meta('display_name',$post->post_author).'</a>';
                                    $hook_out.='</div>';
                                }
                                ...
                                if ($hook_count==0)
                                    $hook_count=1;
                                $hook_out.='<span class="hook_min_read">'.$hook_count.' '.$hook_translated['min_read_text'].'</span>';
                                $hook_out.='</a>';
                                $hook_out.='</div>';
                                $hook_out.='</div>';
                                $hook_out.='<div class="clearfix"></div>';
                    ...
                    if ($hook_query->max_num_pages>1 && $hook_nav_type!='ajaxed') {
                        $big=999999999;
                        $hook_out.='<div id="hook_nav" class="zero_color hook_anchor">';
                        $hook_out.= paginate_links( array(
                            'base' => str_replace( $big, '%#%', esc_url( get_pagenum_link( $big ) ) ),
                            'format' => '?paged=%#%',
                            'current' => max( 1, get_query_var('paged') ),
                            'total' => $hook_query->max_num_pages,
                            'prev_text' =>'<i class="mdi-chevron-double-left"></i>',
                            'next_text' =>'<i class="mdi-chevron-double-right"></i>',
                            'type' => 'list',
                        ));
                        $hook_out.='</div>';
                    }
...
else if (get_field('blog_layout')=="grid") {
    //GRID LAYOUT
    $hook_show_sidebar=$prk_hook_options['right_sidebar'];
    if ($hook_show_sidebar=="1")
        $hook_show_sidebar=true;
    ...
    ?>
    <div id="hook_ajax_inner" class="hook_colored page-prk-blog-grid<?php echo esc_attr($hook_featured_style); ?>" data-color="<?php echo esc_attr($hook_featured_color); ?>">
        <div id="hook_content">
            <?php
            if ($hook_show_title==true)
            {
...
else if (get_field('blog_layout')=="classic") {
    //BIG IMAGES LAYOUT
    $hook_show_sidebar=$prk_hook_options['right_sidebar'];
    if ($hook_show_sidebar=="1")
        $hook_show_sidebar=true;
    ...
    $hook_grid_align=get_field('grid_align');
    ?>
    <div id="hook_ajax_inner" class="hook_colored page-prk-blog-full<?php echo esc_attr($hook_featured_style); ?>" data-color="<?php echo esc_attr($hook_featured_color); ?>">
        <div id="hook_content">
            <?php
            if ($hook_show_title==true)
            {
                        ...
                        while ($hook_query->have_posts()) : $hook_query->the_post();
                            ...
                            if ($hook_post_counter<=$posts_per_page) {
                                ...
                                if ($prk_hook_options['postedby_blog']=="1") {
                                    ...
                                    $hook_out.='<div class="single_blog_meta_div">';
                                    $hook_out.='<div class="prk_lf hook_colored_link hook_author">';
                                    $hook_out.=esc_attr($hook_translated['posted_by_text']).' <a href="'.get_the_author_meta('display_name',$post->post_author).'" class="hook_anchor not_zero_color">'.get_the_author().'</a>';
                                    $hook_out.='</div>';
                                    $hook_out.='</div>';
                                }
                                $hook_out.='<div class="clearfix"></div>';
...
else if (get_field('blog_layout')=="split") {
    //BIG IMAGES LAYOUT
    $hook_show_sidebar=$prk_hook_options['right_sidebar'];
    if ($hook_show_sidebar=="1")
        $hook_show_sidebar=true;
            ...
            <div id="hook_content_inner">
                <div class="small-12 small-centered columns row <?php if (get_field('limit_width')==1 || $hook_show_sidebar==true) {echo "prk_inner_block";}else{echo "hook_unblog";} ?>">
                    ...
                    if ($hook_query->have_posts()) :
                        $hook_post_counter=1;
                        $hook_stop_flag=true;
                        $hook_out='<div class="classic_blog_section hook_split" data-items="'.esc_attr($posts_per_page).'">';
                        $hook_terms=get_terms("category");
                        $hook_count=count($hook_terms);
                        if (get_field('show_filter')=="1") {
                            $hook_filter_array=explode(",",$hook_inside_filter);
                        ...
                        while ($hook_query->have_posts()) : $hook_query->the_post();
                            ...
                            if ($hook_post_counter%2==0) {
                                $col_float=' prk_regular';
                            }
                            else {
                                $col_float=' prk_inverted';
                            }
                            if ($hook_post_counter<=$posts_per_page) {
                                $hook_out.='<div id="post-'.get_the_ID().'" class="blog_entry_li blog_hover '.esc_attr($hook_grid_align.$hook_featured_class.$hook_filters.$col_float).'" data-color="'.esc_attr($hook_featured_color).'">';
                                $hook_ext_count=0;
                                $hook_imgs_width=$prk_hook_options['custom_width']+80*2;
                                $hook_out.='<div class="row"><div class="hook_split_wrap">';
                                $hook_out.='<div class="columns small-7">';
                                if (has_post_thumbnail($post->ID)) {
                                    $hook_ext_count=1;
                                    $hook_image=wp_get_attachment_image_src( get_post_thumbnail_id($post->ID),'');
                                    $hook_out.='<a href="'.get_permalink().'" class="hook_anchor">';
                                ...
                                $hook_out.='</div>';
                                $hook_out.='<div class="small-5 columns">';
                                $hook_out.='<div class="hook_post_info">';
                                $hook_out.='<div class="hook_inn">';
                                $hook_content=get_post_field('post_content',$post->ID);
                                $hook_out.='<div class="entry_title header_font">';
    ...
    $hook_grid_align=get_field('grid_align');
    ?>
    <div id="hook_ajax_inner" class="hook_colored page-prk-blog-stacked<?php echo esc_attr($hook_featured_style); ?>" data-color="<?php echo esc_attr($hook_featured_color); ?>">
        <div id="hook_content">
            <?php
            if ($hook_show_title==true)
            {
                ...
                <div class="small-12 small-centered columns row prk_inner_block">
                    ...
                    if ($hook_query->have_posts()) :
                        ...
                        while ($hook_query->have_posts()) : $hook_query->the_post();
                            ...
                            $hook_imgs_width=$prk_hook_options['custom_width']+80*2;
                            if ($hook_post_counter<=$posts_per_page) {
                                $hook_out.='<div id="post-'.get_the_ID().'" class="blog_entry_li wpb_animate_when_almost_visible wpb_hook_fade_waypoint '.esc_attr($hook_grid_align.$hook_featured_class.$hook_filters).'" data-color="'.esc_attr($hook_featured_color).'">';
                                $hook_out.='<div class="hook_post_info">';
                                ...
                                if ($prk_hook_options['postedby_blog']=="1") {
                                    ...
                                    $hook_out.='<div class="hook_blog_uppercased prk_75_em prk_heavier_600 header_font hook_anchor prk_lf hook_colored_link hook_author">';
                                    $hook_out.=esc_attr($hook_translated['posted_by_text']).' <a href="'.get_author_posts_url($post->post_author).'" class="not_zero_color">'.get_the_author().'</a>';
                                    $hook_out.='</div>';
                                    $hook_out.='</div>';
                                    $hook_out.='<div class="clearfix"></div>';
                                }
                                $hook_out.='<div class="small-10 columns small-centered prk_bordered_bottom"></div>';
                                $hook_out.='</div>';
                                //END POST
                            } else if ($hook_nav_type=='ajaxed') {
                                if ($hook_stop_flag==true)
