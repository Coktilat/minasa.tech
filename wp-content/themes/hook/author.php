...
$hook_author_array = get_user_by('slug', get_the_author_meta('user_nicename'));
if (get_the_author_meta('prk_author_custom_header',$hook_author_array->ID)!="") {
    $hook_featured_style='';
}
else {
    ...
    <div id="hook_ajax_inner" class="page-prk-blog-masonry<?php echo esc_attr($hook_featured_style); ?>">
        <div id="hook_content">
            ...
            if ( have_posts() ) {
                ...
                if (get_the_author_meta('prk_author_custom_header',$hook_author_array->ID)!="") {
                    $hook_vt_image=vt_resize( '', get_the_author_meta('prk_author_custom_header',$hook_author_array->ID),0,0,false , $hook_retina_device );
                    echo '<div id="hook_author_header"><img src="'.esc_url($hook_vt_image['url']).'" width="'.esc_attr($hook_vt_image['width']).'" height="'.esc_attr($hook_vt_image['height']).'" alt="'.esc_attr(hook_alt_tag(false,get_the_author_meta('prk_author_custom_header',$hook_author_array->ID))).'" /></div>';
                }
                ...
                if (function_exists('get_avatar')) {
                    ...
                    if (get_the_author_meta('prk_author_custom_avatar',$hook_author_array->ID)!="") {
                        $hook_vt_image=vt_resize( '', get_the_author_meta('prk_author_custom_avatar',$hook_author_array->ID) ,480 ,0 , false , false );
                        echo '<img src="'.esc_url($hook_vt_image['url']).'" width="'.esc_attr($hook_vt_image['width']).'" height="'.esc_attr($hook_vt_image['height']).'" alt="'.esc_attr(hook_alt_tag(false,get_the_author_meta('prk_author_custom_avatar',$hook_author_array->ID))).'" />';
                    }
                    else {
                        if (get_avatar(get_the_author_meta('email'), '240')) {
                ...
                echo '</div>';
                echo '</div>';
                if ($hook_author_array->description!="") {
                    echo '<div id="author_text" class="small-8 small-centered columns prk_inner_block">';
                    echo '<div class="author_description body_colored">'.nl2br($hook_author_array->description).'</div>';
                    if (function_exists('hook_author_links')) {
                        echo hook_author_links();
                    }
                    echo '</div>';
                }
            }
            ...
            <div id="hook_content_inner">
                <div class="prk_inner_block small-12 small-centered columns row">
                    ...
                    if ($hook_query->have_posts()) :
                        $hook_post_counter=1;
                        $hook_stop_flag=true;
                        $hook_thumbs_margin=16;
                        $hook_out='<div id="blog_masonry_father" data-items="'.esc_attr($posts_per_page).'">';
                        ...
                        if ($hook_stop_flag==false) {
                            ...
                            $hook_out.='<a href="#" class="hook_anchor">';
                            $hook_out.=esc_attr($hook_translated['load_more']);
                            $hook_out.='</a>';
                            $hook_out.='<i class="hook_button_arrow hook_fa-chevron-down"></i>';
                            $hook_out.='</div>';
                    ...
                    if ($hook_query->max_num_pages>1 && $hook_nav_type!='ajaxed') {
                        $hook_out.='<div id="entries_navigation_blog" class="row">';
                        $hook_out.='<div class="navigation hook_anchor small-12 small-centered columns">';
                        $hook_out.='<div id="prk_nav_inner" class="navigation hook_anchor">';
                        $hook_out.='<div class="small-12 prk_heavier_600 header_font small_headings_color smoothed_anchor">';
                        $hook_out.=get_next_posts_link('<div class="prk_lf navigation-previous-blog"><i class="prk_lf hook_fa-angle-left hook_fa-4x prk_lf"></i><div class="prk_lf"><div class="special_heading">'.$hook_page_title.'</div>'.esc_attr($hook_translated['older']).'</div></div>',$hook_query->max_num_pages);
                        $hook_out.=get_previous_posts_link('<div class="prk_rf hook_righted_text navigation-next-blog"><div class="prk_lf"><div class="special_heading">'.$hook_page_title.'</div><div class="blog_naver_right">'.esc_attr($hook_translated['newer']).'</div></div><i class="hook_fa-angle-right prk_lf hook_fa-4x"></i></div>',$hook_query->max_num_pages);
                        $hook_out.='</div>';
                        $hook_out.='<div class="clearfix"></div>';
                        $hook_out.='</div>';
                        $hook_out.='</div>';
