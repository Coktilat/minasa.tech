<?php 
    ...
    if (get_field('featured_header')=="1") {
        ...
        if (prk_check_gutenberg()) {
            $hook_featured_style=' class="hook_gutenberg"';
        }
    }
    else {
        $hook_featured_style=' class="hook_forced_menu"';
        if (prk_check_gutenberg()) {
            $hook_featured_style=' class="hook_forced_menu hook_gutenberg"';
        }
    }
    $hook_inside_filter="";
    ...
    if (get_field('dots_navigation')=="1" && has_nav_menu('prk_main_navigation'))
    {
        ?>
            ...
            <?php
                if (isset($post->ID) && get_post_meta($post->ID,'top_menu',true)!="" && get_post_meta($post->ID,'top_menu',true)!="null") {
                        wp_nav_menu(array(
                            ...
                            'menu_class' => 'hook_dotted unstyled '.$prk_hook_options['menu_font'],
                            'link_after' => '',
