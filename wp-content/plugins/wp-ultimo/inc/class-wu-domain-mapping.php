...
class WU_Domain_Mapping {
  ...
  public function filter_site_url($url, $path, $scheme, $blog_id) {
     ...
     */
    if ($this->is_subdomain || !$site || !$site->mapping || $site->site->path == '/') {
       ...
       */
      if (stristr($url, $site->mapping->get_domain())) {

        return $this->str_lreplace(rtrim($site->site->path, '/'), '', $url);
