...
//    } 
  ...
  function column_value($item) {
    if ($item->type == 'percent') return sprintf(__('%s%s OFF', 'wp-ultimo'), number_format_i18n($item->value, 2), '%');
    else if ($item->type == 'absolute') return sprintf(__('%s OFF', 'wp-ultimo'), wu_format_currency($item->value));
  }
