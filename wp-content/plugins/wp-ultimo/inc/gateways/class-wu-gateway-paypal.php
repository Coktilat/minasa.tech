...
class WU_Gateway_Paypal extends WU_Gateway {
  ...
  public function create_express_checkout($args) {
    ...
    $billingAgreementDetails->BillingAgreementDescription = $args->desc;
    ...
    if ($args->initial_profile) {
       ...
       * Discount Line
       */
      if ($coupon_code && $coupon_code['cycles'] !== false && $this->subscription->get_price_after_coupon_code()) {
      ...
      if ($this->subscription->get_price_after_coupon_code() == 0) {
        $itemAmount = null;
  ...
  public function create_recurring_profile($args) {
    ...
    if ($args->charge_initial_amount) {
      ...
      if (!empty($args->trial)) {
        ...
        $activationDetails->InitialAmount = new BasicAmountType(WU_Settings::get_setting('currency_symbol'), $args->trial->price);
        $args->trial->cycles--;
    ...
    if (!empty($args->trial)) {
      ...
      $trialBillingPeriod->Amount             = new BasicAmountType(WU_Settings::get_setting('currency_symbol'), $args->trial->price);
    ...
    switch ($payment_status) {
      ...
      case 'Processed':
        ...
        if ( $ipn->txn_type == 'recurring_payment' 
           ...
           * @since  1.1.2 Hooks for payments and integrations
           */
          do_action('wp_ultimo_payment_completed', $user_id, $this->id, $ipn->mc_gross);
      ...
      if ($ipn->txn_type == 'recurring_payment_profile_created' || $ipn->txn_type == 'subscr_signup') {
        ...
        if ($ipn->initial_payment_status == 'Completed') {
           ...
           * @since  1.1.2 Hooks for payments and integrations
           */
          do_action('wp_ultimo_payment_completed', $user_id, $this->id, $ipn->initial_payment_amount);
