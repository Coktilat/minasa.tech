 ...
 * Pages Add-ons
 *
 * Handles the addition of the Feature Plugins Page
 * @since  0.0.1
...
class WU_Page_Feature_Plugins extends WU_Page {
   ...
   * @var integer
   */
  public $refresh_time = 6 * HOUR_IN_SECONDS;
  ...
  public function init() {
    ...
    $this->addon_count = get_network_option(null, 'wu-ff-count', 2);

    // Add license key to remote url
    $this->remote_url .= '&license_key=' . WU_Settings::get_setting('license_key', '');
  ...
  public function install_plugin() {
    ...
    if (!check_ajax_referer('wu_install_addon', 'nonce', false)) die(__('Not authorized.', 'wp-ultimo'));
   ...
   */
  public function get_remote_url($action = 'feature_plugins') {

    return add_query_arg('action', $action, $this->remote_url);
  ...
  public function get_remote_addons_count() {
    ...
    $saved_count = get_site_transient('wu_saved_ff_count');

    if ($saved_count === false) {

      $response = wp_remote_get($this->get_remote_url('feature_plugins_count'));
      ...
      if (!is_wp_error($response)) {
        ...
        set_site_transient('wu_saved_ff_count', $body, $this->refresh_time);
  ...
  public function get_remote_addons_list() {
    ...
    $response = wp_remote_get($this->get_remote_url('feature_plugins'));
  ...
  public function output() {
    ...
    update_network_option(null, 'wu-ff-count', count($addons));

    // Render, passing the add-on lists
    WP_Ultimo()->render('meta/feature-plugins', array(
      'addons'        => $addons,
...
} // end class WU_Page_Feature_Plugins;

new WU_Page_Feature_Plugins(true, array(
  'id'         => 'wp-ultimo-feature-plugins',
  'type'       => 'submenu',
  'capability' => 'manage_network',
  'title'      => __('Feature Plugins', 'wp-ultimo'),
  'menu_title' => __('Feature Plugins', 'wp-ultimo'),
));
