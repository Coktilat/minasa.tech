...
class WU_Site_Hooks {
  ...
  public function remove_signup_transients() {
    ...
    if (wp_verify_nonce($_REQUEST['_wpnonce'], 'wu_remove_transients')) {
  ...
  public function add_site_template_separator() {
    ...
    <ul class="subsubsub" id="sites-type-list" style="display: none;">
      <li class="all">
        <a href="<?php echo network_admin_url('sites.php'); ?>" class="<?php echo !isset($_GET['type']) ? 'current' : ''; ?>">
          <?php _e('All Sites', 'wp-ultimo');  ?> <span class="count">(<?php echo $count_all; ?>)</span>
        </a> |
      </li>
      <li class="super">
        <a href="<?php echo network_admin_url('sites.php?type=template'); ?>" class="<?php echo isset($_GET['type']) ? 'current' : ''; ?>">
  ...
  public function handle_template_switching() {
    ...
    if ($_POST['template'] == $current_blog_id) {
      ...
      return WP_Ultimo()->add_message( __('You cannot switch templates using this own site as a base template.', 'wp-ultimo'), 'error' );
