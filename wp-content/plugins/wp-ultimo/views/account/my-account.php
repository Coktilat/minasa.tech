...
elseif (isset($_GET['action']) && $_GET['action'] == 'change-plan' && isset($_POST['changing-plan'])) : 
  ...
  <p class="description"><?php echo sprintf(__('Are you sure you want to change your current plan to <strong>%s</strong>, billed every <strong>%s months</strong>?', 'wp-ultimo'), $new_plan->title, $_POST['plan_freq']); ?></p>


  <form method="post">

    <input type="hidden" name="wu_action" value="wu_change_plan">
    <input type="hidden" id="wu_plan_freq" name="plan_freq" value="<?php echo $_POST['plan_freq']; ?>">
