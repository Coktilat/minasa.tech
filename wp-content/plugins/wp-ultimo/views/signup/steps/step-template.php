...
$have_filter = WU_Settings::get_setting('allow_template_filter', true);
  ...
  <div class="themes-php">
    <div class="wrap">
      ...
      <?php if (apply_filters('wu_step_template_display_header', true)) : ?>
        ...
        <h1><?php echo ($is_shortcode) ? esc_html__( 'Available Templates', 'wp-ultimo' ) : esc_html__( 'Pick your Template', 'wp-ultimo' ); ?>
          ...
          <?php
            ...
            */
            ...
            <div class="theme col-<?php echo $cols; ?>" tabindex="0" aria-describedby="<?php echo $aria_action . ' ' . $aria_name; ?>">
               ...
               * @since 1.6.0
               */
              $preview_href = WU_Settings::get_setting('allow_template_top_bar') ? sprintf('onclick="window.open(\'%s\')"', WU_Site_Hooks::get_template_preview_url($theme['id']) . ($is_shortcode ? '&shortcode' : '')) : sprintf('href="%s" target="_blank"', $theme['actions']['visit']);
              ?>
...
<script id="tmpl-theme" type="text/template">
  ...
  /**
    ...
    * @since 1.6.0
    */
  $preview_href = WU_Settings::get_setting('allow_template_top_bar') ? sprintf('onclick="window.open(\'%s\')"', WU_Site_Hooks::get_template_preview_url('{{ data.id }}') . ($is_shortcode ? '&shortcode' : '')) : sprintf('href="%s" target="_blank"', "{{ data.actions.visit }}");
  ?>
