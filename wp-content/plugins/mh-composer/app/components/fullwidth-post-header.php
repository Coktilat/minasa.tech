...
class MHComposer_Component_Fullwidth_Post_Header extends MHComposer_Component {
	...
	function get_fields() {
		...
		$fields = array(
			...
			'show_excerpt' => array(
				'label'             => esc_html__( 'Show Excerpt', 'mh-composer' ),
				'type'              => 'switch_button',
				'options'           => array(
					'off' => esc_html__( 'No', 'mh-composer' ),
					'on'  => esc_html__( 'Yes', 'mh-composer' ),
				),
				'description'       => esc_html__( 'Here you can define whether to show excerpts.', 'mh-composer' ),
			),
			'show_share' => array(
				'label'             => esc_html__( 'Show Share Icons', 'mh-composer' ),
				'type'              => 'switch_button',
				'options'           => array(
					'off' => esc_html__( 'No', 'mh-composer' ),
					'on'  => esc_html__( 'Yes', 'mh-composer' ),
				),
				'description'       => esc_html__( 'Here you can define whether to show share icons. For this option to work you must have "Mharty - Love it" plugin installed and activated.', 'mh-composer' ),
			),
	...
	function shortcode_callback( $atts, $content = null, $function_name ) {
		...
		$show_share			= $this->shortcode_atts['show_share'];
		...
		MHComposer_Core::set_style( $function_name, array(
			'selector'    => '%%order_class%%, %%order_class%% .mhc_display_social_media_share',
			'declaration' => sprintf(
				'text-align: %1$s;',
		...
		$share = do_shortcode('[mhc_social_media_share module_class="post_share_footer" style="solid" type="icon" color="color" column="three" /]');
		$background_layout = 'dark' === $text_color ? 'light' : 'dark';
		$class = " mhc_bg_layout_{$background_layout} mhc_image_{$featured_placement} mhc_align_{$text_orientation}";
		$output = sprintf(
			...
			'on' === $show_excerpt && '' !== $post_excerpt ? sprintf( '<p class="entry-excerpt">%s</p>',
					$post_excerpt ) : '',
			('on' === $show_share && ( function_exists('mh_loveit') )) ? '<div class="mhc_display_social_media_share mh_share_footer post_share_footer">' . $share . '</div>' : ''
		);
