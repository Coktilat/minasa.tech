...
class MHComposer_Component_Signup extends MHComposer_Component {
		...
		$fields = array(
			'provider' => array(
				...
				'options'         => array(
					...
					'mailpoet3'	  => esc_html__( 'MailPoet3', 'mh-composer' ),
					'mailpoet'	  => esc_html__( 'MailPoet2 (Previous)', 'mh-composer' ),
				),
				'affects' => array(
					'#mhc_feedburner_uri',
					'#mhc_mailchimp_list',
			...
			'mailpoet3_form' => array(
				'label'           => esc_html__( 'Form', 'mh-composer' ),
				'renderer'        => 'mh_mailpoet3_option',
				'description'     => esc_html__( 'Select the desired form. Note: to use this option you need to have "MailPoet3" plugin installed and activated. Create your at least one form to be able to use it.', 'mh-composer' ),
				'depends_show_if' => 'mailpoet3',
			),
			'mailpoet3_count' => array(
				'label'             => esc_html__( 'Show Count', 'mh-composer' ),
				'type'            => 'switch_button',
				'options'         => array(
					'off' => esc_html__( 'No', 'mh-composer' ),
	...
	function shortcode_callback( $atts, $content = null, $function_name ) {
		...
		$mailpoet3_form		 	   	 = $this->shortcode_atts['mailpoet3_form'];
		$mailpoet3_count			 = $this->shortcode_atts['mailpoet3_count'];
		$mailpoet3_count_text		 = $this->shortcode_atts['mailpoet3_count_text'];
		...
		switch ( $provider ) {
			...
			$form = sprintf(
				'<div class="mhc_newsletter_form mhc_mailpoet_form">%1$s%2$s</div>',
				do_shortcode('[mailpoet_form id="'. $mailpoet3_form .'"]'),
				( 'on' === $mailpoet3_count ? sprintf(
					'<div class="mhc_mailpoet_count">%1$s %2$s</div>',
					esc_html( $mailpoet3_count_text ),
					do_shortcode('[mailpoet_subscribers_count]')
				) : '')
			);
		}
