...
class MHComposer_Component_Writer extends MHComposer_Component {
	...
	function shortcode_callback( $atts, $content = null, $function_name ) {
		...
		}elseif('none' !== $users_list){
			$the_user =  get_userdata($users_list);
		}else{
			$the_user = 'none';	
		}
		
		if ( 'off' !== $user_url && 'none' !== $the_user ) {
		...
		$the_user_avatar = '';
		 if ( 'off' !== $user_avatar && 'none' !== $the_user ) {
			...
			$the_user_social = '';
		 if ( 'off' !== $user_social && 'none' !== $the_user ) {
				$the_user_social = sprintf(
					...
					( '' !== $the_user->snapchat ? sprintf( '<a href="%1$s" class="mh-author-net snapchat" title="Snapchat" target="_blank"><i class="icon"></i></a>', esc_url( $the_user->snapchat ) ) : '' ),
					...
					( '' !== $the_user->pinterest ? sprintf( '<a href="%1$s" class="mh-author-net pinterest" title="Pinterest" target="_blank"><i class="icon"></i></a>', esc_url( $the_user->pinterest ) ) : '' ),
					...
					( '' !== $the_user->wordpress ? sprintf( '<a href="%1$s" class="mh-author-net wordpress" title="Wordpress" target="_blank"><i class="icon"></i></a>', esc_url( $the_user->wordpress ) ) : '' )
				);
			}
			...
			$the_user_avatar,
			('none' !== $the_user ? $the_user->display_name : ''),
			( 'off' !== $user_blurb && 'none' !== $the_user && '' !== $the_user->description ?  sprintf( 
			...
			('off' !== $user_url && 'none' !== $the_user ? sprintf( '<a href="%1$s%2$s/">', esc_url( site_url( '/author/')), esc_attr( $the_user->user_login ) ) : ''),
			('off' !== $user_url && 'none' !== $the_user ? '</a>' : ''),
			$class
			);
