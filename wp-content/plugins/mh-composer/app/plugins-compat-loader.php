 ...
 */
class MH_Composer_Plugins_Compat_Loader {
	/**
		...
		foreach ( array_keys( get_plugins() ) as $plugin ) {
			...
			if ( is_plugin_active( $plugin ) ) {
				...
				$plugin_compat_url  = apply_filters(
					"mh_composer_plugins_compat_path_{$plugin_compat_name}",
					MH_COMPOSER_DIR . "plugins/{$plugin_compat_name}.php",
					$plugin_compat_name
				);
...
MH_Composer_Plugins_Compat_Loader::init();
