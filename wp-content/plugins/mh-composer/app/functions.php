...
if ( ! function_exists( 'mh_mailpoet3_option' ) ) :
function mh_mailpoet3_option() {
	$output = sprintf(
		'<select name="mhc_mailpoet3_form" id="mhc_mailpoet3_form"><option value="none"%2$s>%1$s</option>',
		esc_html__( 'Select a form', 'mh-composer' ),
		'<%= typeof( mhc_mailpoet3_form ) !== "undefined" && "none" === mhc_mailpoet3_form ? " selected=\'selected\'" : "" %>'
	);
 	//Require plugin.php to use is_plugin_active() below
 	include_once( ABSPATH . 'wp-admin/includes/plugin.php' ); 
  	if ( is_plugin_active( 'mailpoet/mailpoet.php' ) ) {
		...
		$form = $wpdb->get_results(
		  "
		  SELECT id, name
		  FROM {$wpdb->prefix}mailpoet_forms
		  "
		);
		...
		if ( $form ) {
			foreach ( $form as $cform ) {
				$contact_forms[ $cform->name ] = $cform->id;
				$selected = sprintf(
		   			'<%%= typeof( mhc_mailpoet3_form ) !== "undefined" && "%1$s" === mhc_mailpoet3_form ?  " selected=\'selected\'" : "" %%>',
					esc_attr( $cform->id )
				);
...
function mhc_add_composer_page_js_css(){
	...
	wp_enqueue_script( 'mhc_app_js', MH_COMPOSER_URI .'/js/app.min.js', array( 'jquery', 'jquery-ui-core', 'underscore', 'backbone', 'mhc_admin_js' ), MH_COMPOSER_VER, true );
