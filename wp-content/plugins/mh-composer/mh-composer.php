...
if( ! class_exists( 'MHComposer', false ) ) {
	class MHComposer {
		...
		function __construct() {
			...
			add_action( 'admin_notices', array( $this, 'classic_editor_dependency_admin_notice' ) );
			add_action( 'admin_head', array( $this, 'classic_editor_dependency_admin_head' ) );
		}
		...
		function setup_mh_composer() {
			...
			require_once( MH_COMPOSER_PLUGIN_DIR . 'includes/compatibility/block-editor/block-editor.php' );
		...
		//admin notice
		function classic_editor_dependency_admin_notice() {
			if ( ! current_user_can( 'update_plugins' ) ) {
					return false;
				}
			
			if ( mhc_is_wp_5() && (!file_exists( WP_PLUGIN_DIR . '/classic-editor/classic-editor.php' ) || !is_plugin_active( 'classic-editor/classic-editor.php' ) ) ) : ?>
				<div class="notice notice-info dependency-installer"><p><?php echo '<strong>[' . __( 'Notice', 'mh-composer' ) . ']</strong> ' . __( 'The Classic Editor plugin is required.', 'mh-composer' ); ?> <a class="dependency-installer-button" href="<?php echo esc_url( wp_nonce_url( self_admin_url( 'plugin-install.php?tab=favorites&user=wordpressdotorg&save=0' ), 'save_wporg_username_' . get_current_user_id() ) ); ?>"><?php _e( 'Activate Now', 'mh-composer' ); ?></a></p></div>
			<?php endif;
		}
		//add css style
		function classic_editor_dependency_admin_head() {
			?>
			<style>
			.notice.dependency-installer{
