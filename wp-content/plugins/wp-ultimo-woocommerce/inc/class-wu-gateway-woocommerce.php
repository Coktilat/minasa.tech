...
class WU_Gateway_WooCommerce extends WU_Gateway {
   ...
   */
   public function woocommerce_subscription_hooks() {
    ...
    add_action('wp_loaded', array($this, 'create_subscription_cart'));
  ...
  public function create_transaction_on_renew($order, $subscription) {
   ...
   public function redirect_to_cart_on_integration($url, $type) {
    ...
    if ($type == 'success') {
      ...
      return $this->get_create_subscription_cart_url();
   ...
   /**
    * Gets the URL for the cart creation
    *
    * @since 1.2.0
    * @return string
    */
   public function get_create_subscription_cart_url() {

    return wp_nonce_url( network_site_url(), 'wu-create-cart', 'code') . '&wu-action=create_cart';

   } // end get_create_subscription_cart_url;

   /**
    * Creates a shopping cart with the subscription option desired by the user
    * Redirects the user to the cart, afterwards
    *
    * @since 1.2.0
    * @return void
    */
   public function create_subscription_cart() {

    if ( ! isset($_GET['wu-action']) || $_GET['wu-action'] !== 'create_cart') return;

    if ( ! isset($_GET['code']) || ! wp_verify_nonce($_GET['code'], 'wu-create-cart')) return;
    ...
    $cart_key = wu_create_woocommerce_subscription($subscription);

    // Creates the Subscription on WC
    if ( ! $cart_key ) {
    ...
    /**
     * Adds relevant info to the session
     */
