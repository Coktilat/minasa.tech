<?php
    ...
    //PIRENKO
    global $prk_dux_dir;
    //$prk_dux_dir=get_template_directory().'/inc/modules/ReduxCore';
    $prk_dux_dir=dirname( __FILE__ );
    ...
    if ( ! class_exists( 'ReduxFramework' ) ) {
        ...
        class ReduxFramework {
            ...
            public static function init() {
                ...
                //PIRENKO
                global $prk_dux_dir;
                $dir = Redux_Helpers::cleanFilePath( $prk_dux_dir );
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( empty ( $this->args['footer_credit'] ) ) {
                    $this->args['footer_credit'] = '<span id="footer-thankyou">'.sprintf( __( 'Options panel created using %1$s', 'hook' ), '<a href="'.esc_url( $this->framework_url ).'" target="_blank">'.__( 'Redux Framework', 'hook' ).'</a> v'.self::$_version ).'</span>';
                }

                if ( empty ( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = __( 'Options', 'hook' );
                }

                if ( empty ( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = __( 'Options', 'hook' );
                }
            ...
            // Fix conflicts with WPBakery Page Builder
            public function vc_fixes() {
                if ( redux_helpers::isFieldInUse( $this, 'ace_editor' ) ) {
                    //PIRENKO
                    if (function_exists('hook_outside_register_ace')) {
                        hook_outside_register_ace();
                    }
                }
            }
            ...
            private function _internationalization() {
                 ...
                 * @param string     'hook'  text domain
                 */
                //                $locale = apply_filters( "redux/textdomain/{$this->args['opt_name']}", get_locale(), 'hook' );
                //
                ...
                load_plugin_textdomain( 'hook', false, $basepath.'languages' );
            }
            ...
            public function show( $opt_name, $default = '' ) {
                ...
                if ( ! is_array( $option ) && $option != '' ) {
                    echo hook_output().$option;
                } elseif ( $default != '' ) {
                    echo hook_output().$this->_get_default( $opt_name, $default );
                }
            }
            // show()
            ...
            private function _default_cleanup() {
                ...
                if (false) {//Redux_Helpers::isLocalHost() || ( Redux_Helpers::isWpDebug() ) 
            ...
            private function add_submenu( $page_parent, $page_title, $menu_title, $page_permissions, $page_slug ) {
                ...
                if ( isset ( $test[ $page_parent ] ) ) {
                    ...
                    // wrappers and need to be appened to using add_submenu_varra_page.
                ...
                if ( ! empty ( $this->typography ) && ! empty ( $this->typography ) && filter_var( $this->args['output'], FILTER_VALIDATE_BOOLEAN ) ) {
                    ...
                    if ( $this->args['async_typography'] && ! empty ( $this->typography ) ) {
                        ...
                        <script>
                            ...
                            WebFontConfig['google'] = {families: [<?php echo hook_output().$typography->makeGoogleWebfontString ( $this->typography ) ?>]};
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $event = __( 'moving the mouse over', 'hook' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = __( 'clicking', 'hook' );
                    }

                    // Construct message
                    $msg = sprintf( __( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'hook' ), $event, strtolower( $s ) ).'<br/><br/>'.$url;
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    $default_output = __( 'Default', 'hook' ).": ".substr( $default_output, 0, - 2 );
                }
            ...
            public function ajax_save() {
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce".$this->args['opt_name'] ) ) {
                    echo json_encode( array(
                        'status' => __( 'Invalid security credential.  Please reload the page and try again.', 'hook' ),
                        'action' => ''
                    ) );
                ...
                if ( ! current_user_can( $this->args['page_permissions'] ) ) {
                    echo json_encode( array(
                        'status' => __( 'Invalid user capability.  Please reload the page and try again.', 'hook' ),
                        'action' => ''
                    ) );
                ...
                if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
                    ...
                    if ( ! empty ( $values ) ) {
                        ...
                        echo json_encode( array( 'status' => __( 'Your panel has no fields. Nothing to save.', 'hook' ) ) );
                    }
                }
                if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
                            ...
                            if ( is_array( $arr ) && ! empty( $arr ) ) {
                                foreach ( $arr as $x => $y ) {
                                    if ( strpos( strtolower( $y ), 'redux' ) !== false ) {
                                        $msg = __( '<strong>Redux Framework Notice: </strong>There are references to the Redux Framework support site in your config\'s <code>admin_bar_links</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'hook' );
                    ...
                    if ( ! empty( $this->args['share_icons'] ) ) {
                        foreach ( $this->args['share_icons'] as $idx => $arr ) {
                            if ( is_array( $arr ) && ! empty( $arr ) ) {
                                foreach ( $arr as $x => $y ) {
                                    if ( strpos( strtolower( $y ), 'redux' ) !== false ) {
                                        $msg = __( '<strong>Redux Framework Notice: </strong>There are references to the Redux Framework support site in your config\'s <code>share_icons</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'hook' );
