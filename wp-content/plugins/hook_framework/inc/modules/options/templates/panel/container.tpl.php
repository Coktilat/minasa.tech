...
<div class="redux-container<?php echo esc_attr( $expanded ); ?>">
    ...
    <form method="post" 
        ...
        <input type="hidden" id="redux-compiler-hook"
            name="<?php echo hook_output().$this->parent->args['opt_name']; ?>[compiler]"
            value=""/>
        <?php // $this->parent->args['opt_name'] is sanitized in the Framework class, no need to re-sanitize it. ?>
        <input type="hidden" id="currentSection"
            name="<?php echo hook_output().$this->parent->args['opt_name']; ?>[redux-section]"
            value=""/>
        <?php // $this->parent->args['opt_name'] is sanitized in the Framework class, no need to re-sanitize it. ?>
        <?php if ( ! empty( $this->parent->no_panel ) ) { ?>
            <input type="hidden" 
                name="<?php echo hook_output().$this->parent->args['opt_name']; ?>[redux-no_panel]"
        ...
        <input type="hidden" 
               id="last_tab" 
               name="<?php echo hook_output().$this->parent->args['opt_name']; ?>[last_tab]"
