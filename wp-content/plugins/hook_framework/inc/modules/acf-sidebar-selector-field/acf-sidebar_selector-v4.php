...
class acf_field_sidebar_selector extends acf_field {
	...
	function __construct() {
		...
		$this->label = esc_html__( 'Sidebar Selector', 'hook' );
		$this->category = esc_html__( "Choice",'hook' );
		$this->defaults = array(
			'allow_null' => '1',
			'default_value' => ''
		);
	...
	function create_options($field) {
		...
		?>
		<!-- Allow Null Field -->
		<tr class="field_option field_option_<?php echo hook_output().$this->name; ?>">
			<td class="label">
				<label><?php esc_html_e("Allow Null", 'hook'); ?></label>
			</td>
			<td>
			<?php
			do_action('acf/create_field', array(
				...
				'layout'  =>  'horizontal',
				'choices' =>  array(
					'1' => esc_html__('Yes', 'hook'),
					'0' => esc_html__('No', 'hook'),
				)
			));
	...
	<tr class="field_option field_option_<?php echo hook_output().$this->name; ?>">
		<td class="label">
			<label><?php esc_html_e("Default Value", 'hook'); ?></label>
		</td>
		<td>
		<?php
	...
	function create_field( $field ) {
		...
		?>
		<div>
			<select name='<?php echo hook_output().$field['name'] ?>'>
				<?php if ( !empty( $field['allow_null'] ) ) : ?>
					<option value=''><?php esc_html_e( 'Select a Sidebar', 'hook' ) ?></option>
				<?php endif ?>
				<?php
					foreach( $wp_registered_sidebars as $sidebar ) :
					$selected = ( ( $field['value'] == $sidebar['id'] ) || ( empty( $field['value'] ) && $sidebar['id'] == $field['default_value'] ) ) ? 'selected="selected"' : '';
				?>
					<option <?php echo hook_output().$selected ?> value='<?php echo hook_output().$sidebar['id'] ?>'><?php echo hook_output().$sidebar['name'] ?></option>
				<?php endforeach; ?>
