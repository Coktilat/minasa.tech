...
if (!function_exists('hook_relative_time')){
	function hook_relative_time($a) {
		//get current timehookt
		$b = strtotime("now"); 
		//get timehook when tweet created
		$c = strtotime($a);
		//get difference
		$d = $b - $c;
	...
	class prk_tp_widget_recent_tweets extends WP_Widget {
		...
		public function __construct() {
			parent::__construct(
				...
				'Hook: Twitter Widget', // Name
				array( 'description' => esc_html__( 'Display recent tweets', 'ingrid' ), ) // Args
			);
		}
						...
						if($diff >= $crt || empty($tp_twitter_plugin_last_cache_time)){
							...
							if(!empty($tweets->errors)){
								if($tweets->errors[0]->message == 'Invalid or expired token'){
									echo '<br><strong>'.$tweets->errors[0]->message.'!</strong><br />You\'ll need to regenerate it at Twitter Apps page!'.$after_widget;
								}else{
									echo '<br><strong>'.$tweets->errors[0]->message.'</strong>'.$after_widget;
								}
								return;
							}
					...
					if(!empty($tp_twitter_plugin_tweets)){
						if(function_exists('icl_translate')) {
							if (!isset($instance['username'])) { 
								...
								$instance['username'] = icl_translate( 'hook', 'twitter_widget_username', $instance['username'] ); 
							}
							if (!isset($instance['image_path'])) { 
								$instance['image_path'] = ''; 
							} 
							else { 
								$instance['image_path'] = icl_translate( 'hook', 'twitter_widhook_image_path', $instance['image_path'] ); 
							}
							if (!isset($instance['follow_text'])) { 
								$instance['follow_text'] = ''; 
							} 
							else { 
								$instance['follow_text'] = icl_translate( 'hook', 'twitter_widget_follow_text', $instance['follow_text'] ); 
							}
						} else {
							if (!isset($instance['username'])) { 
								$instance['username'] = ''; 
						...
						if ($instance['username']!="")
						{
							...
							echo '<a class="zero_color" target="_blank" href="http://twitter.com/'.$instance['username'].'"><img src="'.esc_attr($instance['image_path']).'" alt="Twitter" /></a>';
							echo '</div>';
							echo '<div class="prk_twt_author header_font body_colored prk_heavier_600">';
							echo hook_output().$instance['username'];
							echo '</div>';
							echo '<div class="prk_twt_follow"><a class="zero_color" target="_blank" href="http://twitter.com/'.$instance['username'].'">'.$instance['follow_text'].'</a></div>';
							echo '<div class="clearfix"></div>';
							echo '<div class="simple_line"></div>';
							echo '<div class="hook_active_icon prk_lf"><i class="hook_fa-twitter"></i></div>';
						}
							echo' <div class="prk_twt_ul twitter_slider per_init" data-autoplay="true" data-delay="5500">';
							$fctr = '1';
							foreach($tp_twitter_plugin_tweets as $tweet){								
								print '<div class="twt_item" style="float:left;width:100%;"><div class="prk_twt_body"><div class="twt_in">'.convert_links($tweet['text']).'</div><div class="clearfix"></div><span class="twitter_time header_font prk_heavier_600">'.hook_relative_time($tweet['created_at']).'</span><div class="clearfix hide_now"></div><span class="twt_extra_info rtl_hidden"> - </span><span><a class="zero_color twitter_time header_font prk_heavier_600" target="_blank" href="http://twitter.com/'.$instance['username'].'">'.$instance['username'].'</a></span></div></div>';
			...
			public function update($new_instance, $old_instance) {				
				...
				if (function_exists('icl_translate')) { 
					icl_translate('hook', 'widget_title', $instance['title']); 
				 	icl_translate('hook', 'twitter_widget_username', $instance['username']);
				 	icl_translate('hook', 'twitter_widhook_image_path', $instance['image_path']);
				 	icl_translate('hook', 'twitter_widget_follow_text', $instance['follow_text']);
				}
