...
class Decent_Comments_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct( false, $name = 'Hook: Better Comments' );
	}
	...
	function form( $instance ) {
		...
		echo "<p>";
		echo '<label for="' .$this->get_field_id( 'title' ).'">'.esc_html__( 'Title', 'hook' ).'</label>';
		...
		echo "<p>";
		echo '<label class="title" title="'.esc_html__( "The number of comments to show.", 'hook' ) .'" for="' .$this->get_field_id( 'number' ).'">'.esc_html__( 'Number of comments', 'hook' ).'</label>';
		...
		echo '<p>';
		echo '<label class="title" title="'.esc_html__( "Sorting criteria.", 'hook' ) .'" for="' .$this->get_field_id( 'orderby' ).'">'.esc_html__( 'Order by ...', 'hook' ).'</label>';
		...
		echo '<p>';
		echo '<label class="title" title="'.esc_html__( "Sort order.", 'hook' ) .'" for="' .$this->get_field_id( 'order' ).'">'.esc_html__( 'Sort order', 'hook' ).'</label>';
		...
		echo "<p>";
		echo '<label class="title" title="'.esc_html__( "Leave empty to show comments for all posts. To show comments for a specific post only, indicate either part of the title or the post ID. To show posts for the current post, indicate: [current]", 'hook' ).'" for="' .$this->get_field_id( 'post_id' ).'">'.esc_html__( 'Post ID', 'hook' ).'</label>';
		echo '<input class="widefat" id="'.$this->get_field_id( 'post_id' ).'" name="'.$this->get_field_name( 'post_id' ).'" type="text" value="'.esc_attr( $post_id ).'" />';
		echo '<br/>';
		echo '<span class="description">'.esc_html__( "Title, empty, post ID or [current]", 'hook' ).'</span>';
		if ( !empty( $post_id ) && ( $post_title = get_the_title( $post_id ) ) ) {
			echo '<br/>';
			echo '<span class="description"> '.sprintf( esc_html__("Selected post: <em>%s</em>", 'hook' ) , $post_title ).'</span>';
		}
		echo '</p>';
		...
		echo '<label class="title" title="'.esc_html__( "If checked, shows an excerpt of the comment. Otherwise the full text of the comment is displayed.", 'hook' ) .'" for="'.$this->get_field_id( 'excerpt' ).'">'.esc_html__( 'Show comment excerpt', 'hook' ).'</label>';
		echo '</p>';
		
		// max_excerpt_words
		$max_excerpt_words = isset( $instance['max_excerpt_words'] ) ? intval( $instance['max_excerpt_words'] ) : '';
		echo "<p>";
		echo '<label class="title" title="'.esc_html__( "The maximum number of words shown in excerpts.", 'hook' ) .'" for="' .$this->get_field_id( 'max_excerpt_words' ).'">'.esc_html__( 'Number of words in excerpts', 'hook' ).'</label>';
		...
		echo "<p>";
		echo '<label class="title" title="'.esc_html__( "The ellipsis is shown after the excerpt when there is more content.", 'hook' ).'" for="' .$this->get_field_id( 'ellipsis' ).'">'.esc_html__( 'Ellipsis', 'hook' ).'</label>';
		...
		echo '<label class="title" title="'.esc_html__( "Whether to show the author of each comment.", 'hook' ) .'" for="'.$this->get_field_id( 'show_author' ).'">'.esc_html__( 'Show author', 'hook' ).'</label>';
		echo '</p>';
		...
		echo '<label class="title" title="'.esc_html__( "Show the avatar of the author.", 'hook' ) .'" for="'.$this->get_field_id( 'show_avatar' ).'">'.esc_html__( 'Show avatar', 'hook' ).'</label>';
		echo '</p>';

		// avatar size
		$avatar_size = isset( $instance['avatar_size'] ) ? intval( $instance['avatar_size'] ) : '';
		echo "<p>";
		echo '<label class="title" title="'.esc_html__( "The size of the avatar in pixels.", 'hook' ) .'" for="' .$this->get_field_id( 'avatar_size' ).'">'.esc_html__( 'Avatar size', 'hook' ).'</label>';
		...
		echo '<label class="title" title="'.esc_html__( "Show a link to the post that the comment applies to.", 'hook' ) .'" for="'.$this->get_field_id( 'show_link' ).'">'.esc_html__( 'Show link to post', 'hook' ).'</label>';
		echo '</p>';
		...
		echo '<label class="title" title="'.esc_html__( "Show an excerpt of the comment or the full comment.", 'hook' ) .'" for="'.$this->get_field_id( 'show_comment' ).'">'.esc_html__( 'Show the comment', 'hook' ).'</label>';
		echo '</p>';
		
		// taxonomy & terms
		$taxonomy = isset( $instance['taxonomy'] ) ? $instance['taxonomy'] : '';
		echo "<p>";
		echo '<label class="title" title="'.esc_html__( "A taxonomy, e.g. category or post_tag", 'hook' ) .'" for="' .$this->get_field_id( 'taxonomy' ).'">'.esc_html__( 'Taxonomy', 'hook' ).'</label>';
		...
		echo '<br/>';
		echo '<span class="description">'.esc_html__( "Indicate <strong>category</strong> if you would like to show comments on posts in certain categories. Give the desired categories' slugs in <strong>Terms</strong>. For tags use <strong>post_tag</strong> and give the tags' slugs in <strong>Terms</strong>.", 'hook' ).'</span>';
		echo '</p>';
