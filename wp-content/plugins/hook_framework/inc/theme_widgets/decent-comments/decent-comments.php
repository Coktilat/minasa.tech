...
function DC_admin_menu() {
	//if ( function_exists('add_submenu_page') ) {
		//add_submenu_page( 'plugins.php', esc_html__( 'Pirenko: Better Comments Options', 'hook' ), esc_html__( 'Pirenko: Better Comments', 'hook' ), 'manage_options', 'decent-comments-options', 'DC_options');
	//}
}
...
function DC_options() {
	...
	if ( !current_user_can( "manage_options" ) ) {
		wp_die( esc_html__( 'Access denied.', 'hook' ) );
	}
	
	echo
		'<div>' .
			'<h2>' .
				esc_html__( 'Pirenko: Better Comments Options', 'hook' ) .
			'</h2>' .
		'</div>';
	...
	echo
		'<form action="" name="options" method="post">' .		
			'<div>' .
				'<h3>'.esc_html__( 'Settings', 'hook' ).'</h3>' .
				'<p>' .
					'<input name="delete-data" type="checkbox" '.( $delete_data ? 'checked="checked"' : '' ).'/>' .
					'<label for="delete-data">'.esc_html__( 'Delete settings when the plugin is deactivated', 'hook' ).'</label>' .
				'</p>' .
				'<p>' .
					wp_nonce_field( plugin_basename( __FILE__ ), DC_OPTIONS_NONCE, true, false ) .
					'<input type="submit" name="submit" value="'.esc_html__( 'Save', 'hook' ).'"/>' .
				'</p>' .
			'</div>' .
		'</form>';
}
