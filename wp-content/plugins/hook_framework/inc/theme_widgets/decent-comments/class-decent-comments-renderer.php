 ...
 * Comment Renderer.
 */
if (!class_exists('Decent_Comments_Renderer')) {
    class Decent_Comments_Renderer
    {
        ...
        static function init()
        {
            self::$orderby_options = array(
                'comment_author_email' => esc_html__('Author Email', 'hook'),
                'comment_author_url' => esc_html__('Author URL', 'hook'),
                'comment_content' => esc_html__('Content', 'hook'),
                'comment_date_gmt' => esc_html__('Date', 'hook'),
                'comment_karma' => esc_html__('Karma', 'hook'),
                'comment_post_ID' => esc_html__('Post', 'hook')
            );
            self::$order_options = array(
                'ASC' => esc_html__('Ascending', 'hook'),
                'DESC' => esc_html__('Descending', 'hook')
            );
        }
        ...
        static function get_comments($options = array())
        {
            ...
            if (!empty($comments)) {
                ...
                $output .= '<div class="comments_slider decent-comments per_init" data-autoplay="true" data-delay="6000" data-navigation="true">';
                ...
                foreach ($comments as $comment) {
                    ...
                    if ($show_avatar) {
                        $output .= '<span class="comment-avatar hook_anchor">';
                    ...
                    if ($show_author) {
                        $output .= '<span class="comment-author zero_color hook_anchor prk_heavier_600 header_font">';
                        $output .= get_comment_author_link($comment->comment_ID);
                        $output .= '</span>';
                    }

                    if ($show_link) {
                        $output .= '<span class="comment-link zero_color hook_anchor prk_heavier_600 header_font">';
                        $output .= sprintf(
                            _x(' on %s', 'comment-link', 'hook'),
