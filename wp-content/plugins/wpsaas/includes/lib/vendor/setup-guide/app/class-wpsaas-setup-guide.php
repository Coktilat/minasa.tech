<?php
if ( ! class_exists( 'WPSAAS_Setup_Guide' ) ) :
/**
 * Setup guide.
 *
 * @since 1.0.0
 */
class WPSAAS_Setup_Guide {
	...
	public static function setup_actions() {
		...
		if (is_multisite()) {
			add_action( 'network_admin_menu', array( __CLASS__, 'add_admin_page' ) );
			add_action( 'network_admin_menu', array( __CLASS__, 'add_meta_boxes' ) );

			add_action( 'network_admin_enqueue_scripts', array( __CLASS__, 'admin_scripts' ) );
		}
    ...
    public static function add_admin_page() {
		...
		if ( ! get_option( 'astoundify_setup_guide_hidden', false ) ) {
			add_menu_page( 
				sprintf( self::get_string( 'page-title' ), "WPSAAS" ), 
    ...
    public static function admin_scripts() {
		...
		if ( $screen->id != self::get_screen_id() and $screen->id != self::get_screen_id() . '-network') {
			return;
		}
    ...
    public static function step_box( $object, $metabox ) {
		...
		// var_dump( trailingslashit( self::$args[ 'steps_dir' ] ) . $args[ 'step' ] . '.php');
...
<?php  
	...
	public static function output_page_intro_title() {
        echo '<h1>' . sprintf( self::get_string( 'intro-title' ), 'WPSAAS 1.0' ) . '</h1>';
	}
