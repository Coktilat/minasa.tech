...
class Clientside_Toolbar {
	...
	static function action_enqueue_site_toolbar_styles() {
		...
		if ( is_admin_bar_showing() && Clientside::is_themed() ) {
			...
			// Toolbar styling
			wp_enqueue_style( 'clientside-toolbar-css', plugins_url( 'css/clientside-site-toolbar-1.14.1.min.css', __FILE__ ), array(), null );

			// Additional external plugin support, when applicable
			// Elementor
			if ( defined( 'ELEMENTOR_VERSION' ) ) {
				wp_enqueue_style( 'clientside-plugin-support-css-elementor', plugins_url( 'css/plugin-support/clientside-elementor-1.14.1.min.css', __FILE__ ), array(), null );
			}
